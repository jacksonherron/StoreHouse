...
def OSESMO(Modeling_Team_Input=None, Model_Run_Number_Input=None, Model_Type_Input=None,
           ...
           Load_Profile_Input=None, Retail_Rate_Name_Input=None,
           Solar_Profile_Name_Input=None, Solar_Profile_Input=None, Solar_Size_Input=None,
    ...
    Load_Profile_Master_Index = "1"
    Load_Profile_Data = np.array(Load_Profile_Input)
    ...
    Marginal_Emissions_Rate_Evaluation_Data = np.zeros(np.shape(Load_Profile_Data))
    ...
    from services.OSESMO.Import_Retail_Rate_Data import Import_Retail_Rate_Data
    ...
    if Model_Type_Input == "Solar Plus Storage":
        Solar_Profile_Master_Index = "1"
        Solar_Profile_Description = "Generated using PV Watts"
        Solar_PV_Profile_Data = np.array(Solar_Profile_Input)

    elif Model_Type_Input == "Storage Only" or Solar_Profile_Name_Input == "No Solar":
        Solar_Profile_Master_Index = "null"
        Solar_Profile_Description = "No solar"
        Solar_PV_Profile_Data = np.zeros(np.shape(Solar_PV_Profile_Data))


    # Import Utility Marginal Cost Data
    # Marginal Costs are mapped to load profile location
    Generation_Cost_Data = np.zeros(np.shape(Solar_PV_Profile_Data))
    Representative_Distribution_Cost_Data = np.zeros(np.shape(Solar_PV_Profile_Data))
    ...
    for Month_Iter in range(1,13):  # Iterate through all months
        ...
        elif delta_t == (15 / 60) or delta_t == (60/60):
    ...
    Output_Directory_Filepath = os.path.join(Input_Output_Data_Directory_Location)
                                # "Models", "OSESMO", "Model Outputs", \
    ...
    return list(P_ES), Storage_Installed_Cost, Solar_Installed_Cost